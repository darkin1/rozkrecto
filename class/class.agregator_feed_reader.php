...
function feedReader(){ //constructor inicia�ao dos valores por defeitos dos elementos da classe
    $this->feedReader="";
    $this->feedUrl="";
    $this->node=0;
...
function parseFeed(){ //fun�ao parse do xml
    ...
    if(!($fp=@fopen($this->feedUrl,"r"))){
        $errorDefinition="Nie ma odpowiedniego pliku. ".$this->feedUrl;
...
function openTag(&$parser,&$name,&$attribs){ //fun�ao startElement
        if($name){
...
function closeTag(&$parser,&$name){ //fun�ao endElement
    $this->currentTag="";
}

function dataHandling(&$parser,&$data){ //fun�ao characterElement
    if($this->channelFlag){ //para a descri�ao do channel
    ...
    if($this->itemFlag){ //para a descri�ao dos items
    ...
    if($this->imageFlag){ //para a descri�ao da imagem
