...
//2005 Portugal
include("class.rss_feed_reader.php");
...
function getItemDescription2($class="",$item){
	if(isset($this->data['item']['content:encoded'][$item])){
		$html = "<span ";
		if(isset($class)){
			$html .= "class=\"".$class."\" ";
		}
		$html .= ">".str_replace ("</pre>", "</p>", str_replace ("<pre>", "<p>", html_entity_decode($this->data['item']['content:encoded'][$item])));
		//tag <pre> change the attribute with in a cell in table
		$html .= "</span>";
		return $html;
	}
